...
# + Partial CSV on Stop
# + App icon for taskbar/title bar/about dialog
...
# ---- helpers to write a partial CSV on cancel ----
...
# --- Set Windows AppUserModelID so taskbar uses our EXE/icon ---
try:
    ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(
        u"InfiNet.NEJusticeScraper.V2"
    )
except Exception:
    pass
...
# Use the same icon in the title bar (and on some shells, taskbar)
try:
    _icon_path = resource_path(os.path.join("assets", "app.ico"))
...
def show_about():
    ...
    ttk.Label(top, text=f"Credits: Alex Clark, {ORG_NAME}").grid(row=2, column=0, sticky="w", padx=12, pady=(0,2))
